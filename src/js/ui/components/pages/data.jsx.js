        ...
        content: (
            <div>
                ...
                <p>Planet orbits are calculated from their state vectors (position and velocity).
                    ...
                    ephemerides on the fly. Rather, it uses precomputed data: only a number of state vectors have
                    been computed in the included three year time span (Jan 2015 - Dev 2017).</p>

                <p>To keep the size of the download reasonable, the separation between computed points is limited
                    (around 100 points per orbit). Interpolating between these points will introduce an error both
                    in the position and velocity, which will affect the calculated orbits.</p>

                <p>In addition, planets with satellites wobble in their orbit around the Sun. To prevent this from
                    affecting their computed orbit around the Sun, their orbits are calculated using the state vectors
                    of the barycenter of their planetary system. For example, the orbit of the Earth
                    is calculated in this page using the Earth-Moon system barycenter state vectors.</p>

                <p>Again to keep the download size as small as possible, the barycenter position is used here as the
